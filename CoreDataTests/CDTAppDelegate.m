...
- (void) checkoVersionWithSampleDB {
    ...
    if (![[NSFileManager defaultManager] copyItemAtPath:[[NSBundle mainBundle] pathForResource:@"sample" ofType:@"db"] toPath:dbPath error:&error]) {
    ...
    newMetadata[NSStoreModelVersionIdentifiersKey] = [[model versionIdentifiers] allObjects];
    ...
    [NSPersistentStoreCoordinator setMetadata:newMetadata forPersistentStoreOfType:NSSQLiteStoreType URL:sourceURL error:&error];
    sourceMetadata = [NSPersistentStoreCoordinator metadataForPersistentStoreOfType:NSSQLiteStoreType URL:sourceURL error:&error];
    isCompatible = [model isConfiguration:nil compatibleWithStoreMetadata:sourceMetadata];
...
- (void) createPersistentStoreCoordinator {
    ...
    NSDictionary *options = [NSDictionary dictionaryWithObjectsAndKeys:pragmaOptions, NSSQLitePragmasOption, nil];

    NSError *error = nil;
    
    NSString *docDir = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    
    NSURL *url = [NSURL fileURLWithPath:[docDir stringByAppendingPathComponent:@"sample.db"]];
    if ([coordinator addPersistentStoreWithType:NSSQLiteStoreType configuration:nil URL:url options:options error:&error]) {
        ...
        BOOL compat = [model isConfiguration:nil compatibleWithStoreMetadata:sourceMetadata];
        ...
        NSManagedObject *spec = [NSEntityDescription insertNewObjectForEntityForName:@"Spec" inManagedObjectContext:context];
